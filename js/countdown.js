...
const countdown = function(_config) {
  const originDate = moment.utc($(_config.target).getAttribute('data-date'), 'DD-MM-YYYY').format('YYYY-MM-DD');
  const originTime = $(_config.target).getAttribute('data-time');
  const fullTime = originDate + ' ' + originTime;
  const newFullTime = moment.utc(fullTime).local().format('DD-MM-YYYY HH:mm:ss').split(' ');
  const newDate = newFullTime[0];
  const newTime = newFullTime[1];
  const tarDate = newDate.split('-');
